...
int main(int argc, char **argv) {
  ...
  mapManager.addMap("Povo", MAPS_DIR "Povo.jpg",
                    ImPlotPoint{11.148481543, 46.065886358},
                    ImPlotPoint{11.151553543, 46.068958358});
  mapManager.addMap("Vadena", MAPS_DIR "Vadena.jpg",
                    ImPlotPoint{11.309756609, 46.430011962},
                    ImPlotPoint{11.316924609, 46.438203962});
  mapManager.addMap("FSG", MAPS_DIR "FSG.jpg",
                    ImPlotPoint{8.558931763, 49.323440890},
                    ImPlotPoint{8.595726757, 49.335430701});
  mapManager.addMap("Ala", MAPS_DIR "Ala.jpg",
                    ImPlotPoint{11.010747213, 45.784567764},
                    ImPlotPoint{11.013506837, 45.787133634});
  mapManager.addMap("Varano", MAPS_DIR "Varano.jpg",
                    ImPlotPoint{10.013347233, 44.677561879},
                    ImPlotPoint{10.031744730, 44.684102509});
  ...
  static char port_or_file[256] = DEFAULT_GPS_PORT;
  ...
  while (!gui.shouldClose()) {
    ...
    if (ImGui::Begin("ACR", nullptr, window_flags)) {
      ...
      if (ImGui::Button("Load Log File")) {
        if (!fileDialogActive.load()) {
          ...
          std::thread([promisePtr, currentPath]() mutable {
            nfdu8filteritem_t filterList[] = {{"Log files", "log,txt"}};
      ...
      if (ImGui::Button("Load Cones CSV")) {
        if (!conesDialogActive.load()) {
          ...
          std::thread([promisePtr, currentPath]() mutable {
            nfdu8filteritem_t filterList[] = {{"CSV files", "csv"}};
            size_t filterCount = sizeof(filterList) / sizeof(filterList[0]);
      ...
      if (showGPSDialog) {
        if (ImGui::BeginPopupModal("Open GPS", NULL,
          ...
          if (ImGui::Button("OK")) {
            ...
            if (gpsManager.initialize(port_or_file) == -1) {
              printf(
                  "Error: GPS not found or failed to initialize on port %s.\n",
                  port_or_file);
              notificationManager.showPopup(
                  "GPS_Error", "Error",
                  "GPS not found or failed to initialize.",
              ...
              printf("Started reading GPS data from serial port %s.\n",
                     port_or_file);
              notificationManager.showPopup(
                  "GPS", "GPSConnected", "Reading GPS data from serial port.",
        ...
        if (ImGui::BeginTabItem("Settings")) {
          if (ImGui::CollapsingHeader("Map Settings")) {
            ...
            for (size_t i = 0; i < maps.size(); i++) {
              if (ImGui::RadioButton(maps[i].name.c_str(),
                                     &mapManager.selectedMapIndex_, i)) {
                notificationManager.showPopup("MapManager", "Map Selected",
          ...
          if (ImGui::CollapsingHeader("Font Settings")) {
            ...
            if (ImGui::BeginCombo(
                    "Select Font",
                    fontManager
                        .getAvailableFonts()[fontManager.getSelectedFontIndex()]
                        .name.c_str())) {
              const auto &availableFonts = fontManager.getAvailableFonts();
              for (size_t n = 0; n < availableFonts.size(); n++) {
                bool is_selected =
                    (fontManager.getSelectedFontIndex() == static_cast<int>(n));
                if (ImGui::Selectable(availableFonts[n].name.c_str(),
                                      is_selected))
            ...
            if (fontManager.getSelectedFontIndex() >= 0 &&
                fontManager.getSelectedFontIndex() <
                    static_cast<int>(fontManager.getAvailableFonts().size())) {
            ...
            ImGui::SameLine();
            ImGui::SliderFloat("##FontScale", &fontManager.fontScale_, 0.5f,
                               2.0f);
            if (ImGui::IsItemHovered())
          ...
          if (ImGui::CollapsingHeader("Theme Settings")) {
            ...
            if (ImGui::Combo("Select Theme", &selectedThemeIndex, themes,
                             IM_ARRAYSIZE(themes))) {
              ...
              notificationManager.showPopup(
                  "Theme", "Theme Changed",
                  "The application theme has been updated.",
          ...
          if (ImGui::CollapsingHeader("Reset Settings")) {
            if (ImGui::Button("Reset Trajectory and Cones")) {
              ...
              notificationManager.showPopup(
                  "Reset", "Reset", "Trajectory and cones have been reset.",
                  NotificationType::Info);
              printf("Trajectory and cones have been reset.\n");
            }
            if (ImGui::IsItemHovered())
              ImGui::SetTooltip(
                  "Click to reset the trajectory and all loaded cones.");
          }
        ...
        if (ImGui::BeginTabItem("Statistics")) {
          if (ImGui::CollapsingHeader("GPS Accuracy (DOP)")) {
            ...
            if (ImPlot::BeginPlot("DOP Over Time", ImVec2(-1, 300))) {
              ...
              ImPlot::SetupAxes("Time (s)", "DOP", ImPlotAxisFlags_AutoFit,
              ...
              ImPlot::PlotLine("HDOP", timeValues.data(), hdopValues.data(),
                               hdopValues.size());

              ImPlot::SetNextLineStyle(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), 1.5f);
              ImPlot::PlotLine("PDOP", timeValues.data(), pdopValues.data(),
                               pdopValues.size());
          ...
          if (ImGui::CollapsingHeader("PVT Over Time")) {
            ...
            if (ImPlot::BeginPlot("PVT", ImVec2(-1, 300))) {
              ...
              ImPlot::SetupAxes("Speed (m/s)", "Time (s)",
                                ImPlotAxisFlags_AutoFit,
                                ImPlotAxisFlags_AutoFit);
              ...
              ImPlot::PlotLine("Speed", pvtTimes.data(), pvtSpeeds.data(),
                               pvtSpeeds.size());
      ...
      if (ImGui::IsItemHovered())
        ImGui::SetTooltip("Horizontal Dilution of Precision (HDOP) indicates "
                          "the horizontal accuracy of the GPS.");
      ...
      if (ImGui::IsItemHovered())
        ImGui::SetTooltip(
            "Position Velocity Time (PVT) indicates the speed of the GPS.");
      ...
      while (!timeValues.empty() && (plotTime - timeValues.front()) &&
             (plotTime - pvtTimes.front()) > windowSize) {
...

      {
        ...
        if (ImGui::IsKeyPressed(ImGuiKey_T)) {
          if (gpsManager.getSession().active) {
            ...
            notificationManager.showPopup("Session", "Session Stopped",
            ...
            if (csv_session_setup(&gpsManager.getSession(),
              ...
              notificationManager.showPopup("Session", "Error",
            ...
            notificationManager.showPopup("Session", "Session Started",
        ...
        if (ImGui::IsKeyPressed(ImGuiKey_O)) {
          ...
          notificationManager.showPopup("Cone_Orange", "Cone Placed",
                                        "An orange cone has been placed.",
                                        NotificationType::Success);
        }
        ...
        if (ImGui::IsKeyPressed(ImGuiKey_C)) {
          ...
          gpsManager.clearCones();
          notificationManager.showPopup(
              "Cleared", "Cleared", "Trajectory and cones have been cleared.",
              NotificationType::Info);
          printf("Trajectory and cones have been reset.\n");
        }
      ...
      if (ImGui::BeginPopup("Legend Popup")) {
        ...
        if (ImGui::TreeNode("Cones")) {
          ...
          ImGui::BeginChild("ConesList", ImVec2(260, 80), true,
      ...
      if (ImPlot::BeginPlot("GpsPositions", size,
                            ImPlotFlags_Equal | ImPlotFlags_NoTitle |
                                ImPlotFlags_NoLegend)) {
        plotRendered = true;

        const MapInfo &selectedMap =
            mapManager.getMaps()[mapManager.getSelectedMapIndex()];
        ...
        if (resetView) {
          ImPlot::SetupAxisLimits(ImAxis_X1, minLon - margin_x,
                                  maxLon + margin_x, ImPlotCond_Always);
          ImPlot::SetupAxisLimits(ImAxis_Y1, minLat - margin_y,
                                  maxLat + margin_y, ImPlotCond_Always);
          resetView = false;
        }
...

        {
          if (selectedMap.texture != 0) {
            ImPlot::PlotImage(
                ...
                ImVec2(0, 0), ImVec2(1, 1), ImVec4(1, 1, 1, mapOpacity));
          }
        }
        ...
        for (size_t i = 0; i < cones.size(); ++i) {
          ...
          ImVec2 coneScreen =
              ImPlot::PlotToPixels(ImPlotPoint(cones[i].lon, cones[i].lat));
        ...
        if (!isDragging && shiftDown && closestConeIndex != -1 &&
            ImGui::IsMouseClicked(ImGuiMouseButton_Left) &&
            ImPlot::IsPlotHovered()) {
          isDragging = true;
          draggedConeIndex = closestConeIndex;
          notificationManager.showPopup("Drag_Cone_Start", "Drag Started",
        ...
        if (isDragging && ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
          if (draggedConeIndex >= 0 &&
              draggedConeIndex < static_cast<int>(cones.size())) {
        ...
        if (isDragging && ImGui::IsMouseReleased(ImGuiMouseButton_Left)) {
          if (draggedConeIndex >= 0 &&
              draggedConeIndex < static_cast<int>(cones.size())) {
            notificationManager.showPopup("Drag_Cone_End", "Drag Completed",
        ...
        if (ImPlot::IsPlotHovered() &&
          ...
          if (closestConeIndex != -1 &&
              coneVisibility[closestConeIndex].value) {
        ...
        for (size_t i = 0; i < cones.size(); ++i) {
          if (coneVisibility[i].value) {
            ...
            ImPlot::PlotScatter(coneLabel.c_str(), &cones[i].lon, &cones[i].lat,
                                1);
          }
        }
        ...
        if (showCurrentPosition &&
            (currentPos.x != 0.0f || currentPos.y != 0.0f)) {
          constexpr auto col = ImVec4(1.0f, 0.0f, 0.0f, 1.0f);
          ImPlot::SetNextMarkerStyle(ImPlotMarker_Circle, 8,
                                     ImVec4(0.0, 0.0, 0.0, 0.0), 3.0f, col);
          ImPlot::PlotScatter("Current Position", &currentPos.x, &currentPos.y,
                              1);
        }
      ...
      if (mapManager.showConeContextMenu_ &&
          ...
          mapManager.selectedConeIndex_ <
              static_cast<int>(gpsManager.getCones().size())) {
      ...
      if (ImGui::BeginPopup("Cone Menu")) {
        ...
        if (ImGui::Combo("Color", &selected_color, color_options,
                         IM_ARRAYSIZE(color_options))) {
          ...
          notificationManager.showPopup("Cone_Color_Changed", "Color Changed",
                                        "The cone's color has been updated.",
                                        NotificationType::Success);
        ...
        if (ImGui::Button("Delete")) {
          ...
          notificationManager.showPopup("Cone_Deleted", "Cone Deleted",
                                        "The selected cone has been deleted.",
                                        NotificationType::Info);
    ...
    if (fileDialogActive.load()) {
      if (fileDialogFuture.valid()) {
        if (fileDialogFuture.wait_for(std::chrono::seconds(0)) ==
          ...
          if (!selectedFilePath.empty()) {
            ...
            if (gps_interface_open_log_file(&gpsManager.getGPS(),
                                            selectedFilePath.c_str()) == -1) {
              printf("Error opening selected file: %s\n",
                     selectedFilePath.c_str());
              notificationManager.showPopup("FileBrowser", "Error",
              ...
              notificationManager.showPopup("Success", "Success",
    ...
    if (conesDialogActive.load()) {
      if (conesDialogFuture.valid()) {
        if (conesDialogFuture.wait_for(std::chrono::seconds(0)) ==
            std::future_status::ready) {
          ...
          if (!selectedConesPath.empty()) {
            ...
            if (conesLoader.loadFromCSV(selectedConesPath, loadedCones)) {
              ...
              notificationManager.showPopup(
                  "Cones_Loaded", "Success",
                  "Successfully loaded cones from CSV.",
                  NotificationType::Success);
              printf("Cones loaded from CSV: %s\n", selectedConesPath.c_str());
              resetView = true;
            } else {
              notificationManager.showPopup(
                  "Cones_Load_Error", "Error",
                  "Failed to load cones from the selected CSV file.",
                  NotificationType::Error);
