...
int GPSManager::initialize(const char *port_or_file) {
  ...
  case Utils::open_mode_log_file:
    res = gps_interface_open_log_file(&gps_, port_or_file);
    break;
    ...
    if (protocol == GPS_PROTOCOL_TYPE_SIZE) {
      ...
      if (fail_count > 10) {
        ...
        notificationManager_.showPopup(
            ...
            "GPS disconnected or unable to read data.",
            NotificationType::Error);
        return;
      }
      continue;
...
std::vector<cone_t> &GPSManager::getCones() { return cones_; }

void GPSManager::setConePlacementMode(bool mode) { conePlacementMode_ = mode; }

bool GPSManager::getConePlacementMode() const { return conePlacementMode_; }

gps_serial_port &GPSManager::getGPS() { return gps_; }

std::mutex &GPSManager::getRenderLock() { return renderLock_; }

cone_session_t &GPSManager::getConeSession() { return cone_session_; }

full_session_t &GPSManager::getSession() { return session_; }
...
void GPSManager::setOpenMode(Utils::open_mode mode) { open_mode = mode; }
